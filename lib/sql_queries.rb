...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "SELECT projects.title, SUM(pledges.amount) - projects.funding_goal AS amount_over FROM pledges JOIN projects ON pledges.project_id = projects.id WHERE amount_over >= 0;"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_summed_amount
  "SELECT users.name, SUM(pledges.amount) FROM pledges INNER JOIN users ON users.id = pledges.user_id GROUP BY users.name ORDER BY pledges.amount;"
end
